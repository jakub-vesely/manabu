...
#pragma udata
#endif
static unsigned char out_buffer[32];
...
void Response(unsigned char * answer, unsigned answerLength)
{
	...
	for (; i < answerLength; i++)
		out_buffer[i+1] = answer[i]; 
	PutUsbData(&out_buffer[0], out_buffer[0]);
}
...
void usbDataReaded(unsigned char const *data, int size)
{
	...
	switch (functionId)
	{
		...
		case FID_COMMAND_FLASH_ADDRESS:
			ResponseChar(0);
			PutCommandI2C(COMMAND_FLASH_ADDRESS, data+3, 2);
			break;
		case FID_COMMAND_FLASH_LATCH_WORD:
			PutCommandI2C(COMMAND_FLASH_LATCH_WORD, data+3, 2);
			ResponseChar(0);
			break;
		case FID_COMMAND_FLASH_WRITE_WORD:
			PutCommandI2C(COMMAND_FLASH_WRITE_WORD, data+3, 2);
			ResponseChar(0);
			break;
		...
		case FID_COMMAND_FLASH_LOAD_CHECK:
			PutCommandI2C(COMMAND_FLASH_LOAD_CHECK, NULL, 0);
			ResponseChar(0);
			break;
	}
}
